...
### Native query mapping (KHÔNG dùng `Object[]`)

- Khi dùng native SQL (`EntityManager.createNativeQuery(...)`) **không được** trả `List<Object[]>`/`Object[]` rồi `for` để convert sang DTO.
- Bắt buộc **quy về native query của Hibernate** để map kiểu dữ liệu rõ ràng bằng `addScalar(...)` (ví dụ `unwrap(NativeQuery.class)` + `addScalar` + transformer/mapper).
- Kết quả native query phải trả về **DTO/record/typed object** (hoặc `Map` đã được build từ typed result), tránh index-based `row[0]`, `row[1]`, ...
...
- **Repository paging rule (NEW)**:
    - For repository methods that return `Page<...>` (including autocomplete pages), prefer a `default` method that accepts `(EntityManager, request, Pageable)` and **builds JPQL dynamically**.
    - Only append `where` conditions when the corresponding request field is **non-null** (and `StringUtils.hasText` for strings, `CollectionUtils.isEmpty` for lists).
    - Prefer setting parameters via `TypedQuery#setParameter` (avoid SpEL `@Query` for pageable searches to prevent DB type binding issues, especially with date/time fields).
    - Default paging sort should be **`order by entity.createdDate desc`** unless the task explicitly requires a different ordering.
    - Services should inject `EntityManager` (constructor injection) and call the repository `default` method.
...
- Prefer `java.util.Objects` helpers for null/equality checks (e.g. `Objects.nonNull(...)`, `Objects.isNull(...)`, `Objects.equals(a,b)`) instead of `x != null` / `a.equals(b)` when writing new/updated code in this repo.
...
- Frontend tasks (UI theo dự án): `pages/project/project-tasks/*`; API + model: `pages/project/services/task.service.ts`, `pages/project/models/task.model.ts`, `task.types.ts`, `task-search.request.ts`
...
- `/dashboard`
- `/project` (kèm `/project/:projectId/tasks`, `/project/:projectId/team`)
- `/resource-allocation`
- `/kanban`
- `/management/users`
...
5. `pages/project/services/task.service.ts`
